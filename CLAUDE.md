...
**Implementation status:** the attended PLANNING phase works end to end — `duet new --spec <draft> [--framing <file>]` runs the SPEC review loop to the Commit-spec gate; `duet continue` crosses gates and answers queued flags through the PLAN loop to the Plan-approval gate (`src/cli.ts`, harness in `src/harness/`, workers in `src/providers/`, run state under `.duet/runs/<id>/`). The AFK IMPLEMENTATION and FINAL REVIEW phases, inter-phase worker compaction, PLANNING's onboard/frame/synthesize front half, `--tmux`, and notifications are not built. The `ask_human` pause is **cooperative** (the tool handler persists the question and instructs the orchestrator to end its turn; the SDK's mechanical pauses corrupt resume — Q11 and `src/spike/repro-*.ts`). Stack: XState v5 (Q15), execa 9, zod 4, commander, smol-toml, `@anthropic-ai/claude-agent-sdk`, `@openai/codex-sdk` (pinned alongside the codex CLI version).
...
- Extending the harness to the AFK IMPLEMENTATION phase (new machine states + queued-flag handling on the existing driver machinery), then FINAL REVIEW.
...
- Adding new design questions (Q18+) to `docs/open-questions.md` as operational notes accumulate; recording resolved ones in place with the strike-through convention.
