...
- `src/providers/` — the worker seam: contract, claude + codex adapters, factory; the claude provider has two transports — headless `claude -p` and the opt-in interactive TUI driver (`interactive-claude.ts` parser/locator/worker + the `pane.ts` `PaneController` injection seam; `docs/interactive-transport.md`). `src/run-store.ts` — run-dir persistence (atomic), the CLI↔driver input-staging handshake, and the steer store (file-per-steer, rename-consume). `src/framing.ts` — template, editor flow, frontmatter boundary. `src/status.ts` — the status model + its two renderers (human text, `status --json` verbatim; schema additive-only). `src/cli.ts` — wiring only; parses under `import.meta.main`, so the command table is importable. View glue: `colorize.ts`, `tmux-view.ts`, `notify.ts`.
