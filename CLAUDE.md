...
Status: full arc implemented (`new` → FRAME → SPEC → PLAN → AFK IMPL → Ship gate → DOCS → PR → opened PR), plus the concierge package (mid-phase `steer`, `status --json`, the shipped `skills/duet-concierge/`). FRAME→Ship live-verified (a scratch run + the first real planlab run, ~$93 claude-side); docs/pr/open phases uncrossed; gate pre-authorization awaits its first overnight run (Q20); the concierge package awaits its first live remote session. An opt-in interactive-Claude implementer transport (flat-quota billing) is built as a spike, awaiting its live-auth gate (`docs/interactive-transport.md`). The orchestrator kernel is now host-neutral and drivable over a stdio MCP boundary at behavioral parity — Stage 0 of Claude-Code-as-orchestrator (`docs/future-directions.md` §A), test-proven, not yet live-verified; the live interactive orchestrator is the deferred Stage 1. Codex-as-orchestrator deliberately unbuilt (Q17).
...
- `node src/cli.ts` — the CLI: `new [--spec][--framing][--template][--gates-at <phases|overnight|skip-plan>][--tmux]` (bare = $EDITOR on a template-seeded `.duet/framing-draft.md`; `--template <name>` seeds from project `.duet/templates/<name>.md`, bare prefers `.duet/templates/default.md`) / `continue [--approve ["rider"]|--reject ["…"]|--answer "…"]` (an approval may carry a rider; bare `--approve`/`--reject` opens $EDITOR for the text) / `steer "<note>"` (the mid-phase human voice — live or crashed phases only) / `status [--json][--wait]` / `runs` / `view` / `logs` / `takeover <role>` / `abandon [--purge]` (kill a run's driver and mark it abandoned — reversible; `--purge` also deletes its session transcripts). `duet --help` carries the run model (the CLI is self-documenting — the concierge skill leans on it). Commands return immediately; phases run in a detached `_drive` child (stdout → `.duet/runs/<id>/driver.log`, pid-guarded); with pre-authorized gates the driver lives through the whole stretch to the next attended stop.
...
- `src/harness/` — `machine.ts` (statechart: a phase emits an internal `phase.*` event; gates cross only on `human.*` — two disjoint vocabularies), `phase-events.ts` (that vocabulary + the pure, phase-matched marker→event read), `tools.ts` (the 7 orchestrator tools as a host-neutral `KernelTool` registry + protocol rails; every phase-continuing result also delivers pending steers), `driver.ts` (the in-process host: Agent SDK session per phase, resolves the `phase.*` event, steer carry-forward; `toSdkTools` adapter), `mcp-server.ts` (the stdio-MCP adapter + the hidden `duet _mcp` harness body) + `stdio-host.ts` (the out-of-process driver + the `Orchestrate` seam — the same kernel at parity), `lifecycle.ts` (detached driver + `killDriver` + `gates_at` auto-cross + the spent-marker guard + `probeRunPosition` — where a run is: running/gate/flag/crashed/abandoned/done, derived from driver liveness + parked snapshot + state evidence, never the snapshot alone), `orchestrator-prompts.ts`.
- `src/providers/` — the worker seam: contract, claude + codex adapters, factory; the claude provider has two transports — headless `claude -p` and the opt-in interactive TUI driver (`interactive-claude.ts` parser/locator/worker + the `pane.ts` `PaneController` injection seam; `docs/interactive-transport.md`). `src/run-store.ts` — run-dir persistence (atomic), the CLI↔driver input-staging handshake, the steer store (file-per-steer, rename-consume), and run teardown (`duet abandon`: the abandoned marker + `--purge`). `src/sessions.ts` — locating a run's provider session transcripts by id; the one module reaching outside `.duet/` (what `--purge` deletes). `src/framing.ts` — framing-draft seed selection (built-in template or project `.duet/templates/`), editor flow, frontmatter boundary. `src/status.ts` — the status model + its two renderers (human text, `status --json` verbatim; schema additive-only). `src/cli.ts` — wiring only; parses under `import.meta.main`, so the command table is importable. View glue: `colorize.ts`, `tmux-view.ts`, `notify.ts`.
