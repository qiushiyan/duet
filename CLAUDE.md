...
- `node src/cli.ts` — the CLI: `new [--spec][--framing][--gates-at <phases|overnight>][--tmux]` (bare = $EDITOR on template `.duet/framing-draft.md`) / `continue [--approve|--reject "…"|--answer "…"]` / `steer "<note>"` (the mid-phase human voice — live or crashed phases only) / `status [--json]` / `runs` / `view` / `logs` / `takeover <role>`. Commands return immediately; phases run in a detached `_drive` child (stdout → `.duet/runs/<id>/driver.log`, pid-guarded); with pre-authorized gates the driver lives through the whole stretch to the next attended stop.
