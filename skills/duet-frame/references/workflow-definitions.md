...
Two authoring layers, no npm install: `.duet/workflows/<name>.ts` for a project shape (add `!/workflows/` to `.duet/.gitignore` to commit it), `~/.config/duet/workflows/<name>.ts` for one the user wants across repos. duet provisions each dir with a `tsconfig.json` and a typed SDK stub, so the editor typechecks the file as-is. The filename must match the `name:` inside, and one name may live in only one layer — a name defined twice (including redefining a shipped name, as examples 1 and 2 do) is a load error, never a silent shadow.
