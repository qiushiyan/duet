...
- `doc(artifact, { rounds?, contract?, audit?, name? })` — a maker/checker review loop over a committed document; `artifact` is `'spec' | 'plan'`. A workflow's first document is always the `spec` (half-technical: product goals plus module shape, seams, and test standards); any later one is a `plan` (the tactics the spec deferred).
