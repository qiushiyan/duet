...
- the **evaluation loop, end to end** — the **run corpus** (the series' three records mirrored live, transcripts captured gzipped), **decision grading** (`duet grade`'s first real sessions produced the first instrumented triage-precision numbers — over-flag 0%, under-flag 14% — and caught a real classification bug the same day, PR #39), the **`duet graph`** / **`duet stats --trace`** views pointed at live runs, **inline provider tuning** (every series framing carried explicit per-binding models + efforts, preflighted at creation), and corpus **replay**'s first slice reconstructing a real record dry-run — the method lives in [`docs/corpus-runbook.md`](docs/corpus-runbook.md)
