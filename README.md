...
**Working today — PLANNING through the AFK IMPLEMENTATION phase.** In a project repo, `duet new --spec <draft> [--framing <file>]` starts an orchestrator-driven SPEC review loop and exits at the Commit-spec gate; `duet continue [--approve | --reject "…" | --answer "…"]` crosses gates and answers queued flags — through the PLAN loop to the Plan-approval gate (the walk-away point), then the AFK IMPLEMENTATION phase (plan committed, slices, midpoint checkpoint and compaction at orchestrator judgment, handoff, review rounds, CEO summary) to the Ship gate; `duet status` / `duet runs` inspect. Every quiescent stop pings a macOS notification. Run state lives under `.duet/runs/<id>/` (state hint, machine snapshot, one log per voice, notes file). The PLANNING half is live-verified (`docs/open-questions.md` Q14, ~$4 claude-side per planning run at Opus); the AFK half is built and smoke-tested but awaits its first real run.

**Not built yet:** the FINAL REVIEW phase (approving the Ship gate ends the run; verification, docs, and the PR are manual), the onboard/frame/synthesize front half of PLANNING, `--tmux` viewing, and codex-as-orchestrator (Q17).
