...
One optional binding sits outside the shipped default: add `[roles.consultant]` (or pass `--consultant <provider[:model]>` per run) to enable the **consultant**, an off-by-default second reviewer — pick a different model family from your reviewer, which is the point. `--no-consultant` disables a configured one for a single run.
