...
describe('voice-log stamped blocks', () => {
  test('parses headers and bodies without the appendVoiceLog separator', () => {
    const log = [
      entry(0, '◀ harness prompt (phase=design)', 'line 1\nline 2'),
      entry(1, '▶ orchestrator'),
      entry(2, 'ask_human queued', 'Need a decision?\nWith context.'),
    ].join('');

    expect(parseVoiceLogBlocks(log)).toEqual([
      { stamp: stamp(0), header: '◀ harness prompt (phase=design)', body: 'line 1\nline 2' },
  ...
  test('exports the protocol marker vocabulary used by stats and replay readers', () => {
    expect.soft(PHASE_OPEN_HEADER.exec('◀ harness prompt (phase=design)')?.[1]).toBe('design');
    expect.soft(PHASE_CLOSE_HEADER.exec('advance_phase (design)')?.[1]).toBe('design');
    expect.soft(WORKER_PROMPT_HEADER.exec('◀ prompt (tag=review-design, from orchestrator)')?.[1]).toBe('review-design');
