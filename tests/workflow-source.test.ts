...
import { projectWorkflowDir, resolveWorkflowSource, userWorkflowDir, workflowSdkRuntimeUrl } from '../src/surfaces/workflow-source.ts';
...
`,
  ...
  test('provisions both authoring layers when an external workflow name is missing', async ({ projectDir }) => {
    const home = join(projectDir, 'home');
    await expect(resolveWorkflowSource(projectDir, 'missing-workflow', { home })).rejects.toThrow(/workflow "missing-workflow" was not found/);

    const projectTsconfig = readFileSync(join(projectWorkflowDir(projectDir), 'tsconfig.json'), 'utf8');
    const userDts = readFileSync(join(userWorkflowDir(home), 'duet-workflows.d.ts'), 'utf8');
    expect.soft(projectTsconfig).toContain('"erasableSyntaxOnly": true');
    expect.soft(projectTsconfig).toContain('"duet/workflows"');
    expect.soft(userDts).toContain('Generated by duet');
    expect.soft(userDts).toContain("declare module 'duet/workflows'");
  });
