...
describe('scripts/corpus/replay-phase', () => {
  test('without --yes, prints the live-token gate and does not invoke the orchestrator turn', async ({ blueprintRun, replayOutDir }) => {
    ...
    const result = await runReplayPhaseCli(
      ['--record', blueprintRun.runId, '--phase', 'design', '--out', replayOutDir, '--json'],
      {
        runTurn,
    ...
    expect.soft(lines).toContain(`output: ${replayOutDir}`);
    expect.soft(lines.join('\n')).toContain('fresh live orchestrator SDK session');
    expect.soft(lines.join('\n')).toContain('--yes');
  });

  test('dry-run parses and rewinds without invoking the orchestrator turn', async ({ blueprintRun, replayOutDir }) => {
    const recordDir = runDirOf(blueprintRun.cwd, blueprintRun.runId);
    writeFileSync(
    ...
    const result = await runReplayPhaseCli(
      ['--record', blueprintRun.runId, '--phase', 'design', '--out', replayOutDir, '--dry-run', '--json'],
      {
        runTurn,
        stdout: (line) => lines.push(line),
        loadRecords: () => ({
    ...
    expect.soft(parsed.replayRunDir).toContain(replayOutDir);
