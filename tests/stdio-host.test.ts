...
import { runPhaseOverStdio, stdioPhaseMachine } from '../src/orchestrator/hosts/stdio-host.ts';
import type { Orchestrate } from '../src/orchestrator/hosts/stdio-host.ts';
 ...
 *
 * Only transport-unique behavior lives here. The shared run-loop rails (entry
 * marker-replay, nudge-once, twice-ended flag) are pure on-disk state semantics
 * the transport cannot alter — tests/host-runner.test.ts owns them.
 *
