...
import { z } from 'zod';
import { acceptanceContractPathForSpec, checkerDutyOf, fixerDutyFor, phaseSpec, stageOf } from '../registry/workflows.ts';
import type { PhaseName } from '../registry/workflows.ts';
import { providerFor } from '../voices/providers/index.ts';
import { BudgetCutoffError } from '../voices/providers/types.ts';
import type { VoiceAddress, WorkerProviders, WorkerTurn } from '../voices/providers/types.ts';
import { CONTEXT_CAUTION_PERCENT, CONTEXT_EMERGENCY_PERCENT, contextBand, latestTranscriptUsageTokens, salvageCompactInstructions } from '../voices/context.ts';
import { countsReviewRound, liveContinuityEdgeFor, orphanRecoveryFor, phaseAddressesFor, sessionIdFor, sessionKeyFor, sessionPolicyFor, sessionRecordFor, sessionSlotsToReset, shouldResetAfterCompactAbort, writeAuthorityFor } from '../voices/policy.ts';
import { getSnippet, renderSnippetLibrary, runtimeLibraryContext } from './library.ts';
import {
  appendNote,
  appendVoiceLog,
  clearContextUsage,
...
import { readTranscriptTailAtPath, readTranscriptTailForSession } from '../voices/sessions.ts';
import type { TurnDispatcher } from './hosts/turn-dispatcher.ts';
import { classifyError, formatAge, probeRole } from '../voices/health.ts';
import { activityLine, latestActivity, repoRelative } from '../voices/activity.ts';
import {
  answerResumePrompt,
  approvalRiderBlock,
  buildPhaseBrief,
