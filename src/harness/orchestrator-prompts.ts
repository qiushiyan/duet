...
A persistent session's other cost is its context window: a claude-bound worker's fills as it works, and nothing empties it unless you compact — left alone it eventually rejects every prompt. The harness meters it for you: each worker result ends with the role's fill ([context N% · …]), and the reading stays live even through a long turn. Read it like a fuel gauge. From 75%, a compaction is due — at the next natural pause, send that worker a body that is literally "/compact " followed by your adapted compact-for-* instructions; compaction is cheaper and keeps more headroom the earlier it runs. At 85% the harness enforces the line: normal sends to that role are refused until a /compact goes through, and a running turn is cut before the session can overflow — each refusal or cut prescribes its own recovery. If a session is somehow already rejecting prompts, the harness runs one generic salvage compaction itself, and your failed send's result says what happened and the next step. codex-bound workers compact themselves — none of this applies to them.
