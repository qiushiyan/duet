...
A persistent session's other cost is its context window: a claude-bound worker's fills as it works and never compacts itself — left alone it eventually rejects every prompt, including the recovery. The harness meters it for you: each worker result ends with the role's fill ([context N% · …]), kept honest through long turns by mid-turn sampling. Read it like a fuel gauge. From 75%, a compaction is due — at the next natural pause, send that worker a body that is literally "/compact " followed by your adapted compact-for-* instructions; compaction is cheaper and keeps more headroom the earlier it runs. At 85% the harness steps in structurally: it refuses normal sends to that role until a /compact goes through, and cuts a running turn before the session can overflow — the result then prescribes the recovery (compact, then resume). If a session is somehow already rejecting prompts, the harness runs one generic salvage compaction itself and the result says what happened: re-send after a successful salvage; re-anchor a reset session with recover-context. codex-bound workers compact themselves — none of this applies to them.
