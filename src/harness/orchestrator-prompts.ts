...
3. Drive the implementation as a single pass, not a slice-by-slice loop with reviews between. Send the implementer one prompt to implement the whole plan — every slice, end to end — one commit per slice with that slice's tests per the plan's verification story. The plan already fixes the slice order and verification, so the implementer executes it straight through; a review or a deliberate hold between slices burns a slow worker turn re-covering ground the post-implementation review (step 6) covers anyway. Implementation crosses turn boundaries only because a worker turn is budget- and time-bounded: when the implementer stops partway (low budget, or the turn ran long), send a short continue prompt to pick up the remaining slices from where it left off — that is resumption, not a checkpoint, so don't insert a review or a re-plan between those turns. Worker budget is per-turn — each prompt carries a fresh ceiling — so a low-budget report means continue next turn, never shrink the scope: descoping is a product decision that needs work-content reasons and an honest line in the Ship packet. Have the implementer keep ephemeral verification harnesses (throwaway tsconfigs, scratch scripts) under .duet/scratch/ or delete them before handoff, so they don't ride the worktree as untracked strays.
