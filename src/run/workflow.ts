...
export function workflowPath(cwd: string, runId: string): string {
  return join(cwd, '.greenflag', 'runs', runId, WORKFLOW_FILE);
}
...
/**
 * greenflag keeps no backward compatibility for retired workflow vocabulary, so a
 * frozen workflow that no longer validates IS a run greenflag can no longer speak —
 ...
 * record that silently vanished from `greenflag stats` / `grade` / `graph` would read
 ...
 * greenflag stopped speaking — a `design` artifact kind, a workflow-named build
...
function readWorkflowFile(state: Pick<RunState, 'runId' | 'workflow'>, path: string): CompiledWorkflow {
  ...
  try {
    ...
    throw new UnloadableRunError(
      state.runId,
      `run ${state.runId} froze a workflow greenflag no longer speaks — ${error instanceof Error ? error.message : String(error)}. greenflag keeps no backward compatibility for retired workflow vocabulary; runs frozen before 2026-07-08 (when the spec and design documents unified into one spec doc-loop) are the known cohort. Its transcripts and logs are intact: read them directly, or remove .greenflag/runs/${state.runId}. Replay and grading of these runs is not supported.`,
    );
  }
  if (workflow.name !== state.workflow) {
    throw new Error(
...
export function workflowFor(state: Pick<RunState, 'cwd' | 'runId' | 'workflow'>): CompiledWorkflow {
  return workflowForRunDir(state, join(state.cwd, '.greenflag', 'runs', state.runId));
}
