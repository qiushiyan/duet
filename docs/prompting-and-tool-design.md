...
The rulebook's cold-reader test (anchor identity, then the familiar-term test) is duet's most repeatedly-earned rule, in both directions:

**Under-supply — the missing identity anchor.** Every surface duet ships is read standalone; the shipped skills bite hardest, since a Claude Code session loads one with nothing else around it. **(observed:** the `duet-frame` skill's first draft opened "a duet framing is…" and never said duet is a CLI that orchestrates a pair of worker agents from a framing — obvious to the author mid-build, opaque to the cold reader; fixed with a one-line "what duet is" anchor.**)**
...
Duet's orchestrator surface follows the few-tools rule — `send_prompt` hides spawn/resume/stream/persist behind one verb, and `get_task` is the single way in to a phase (the brief, plus any staged human input folded once), so the interactive host re-anchors through one call rather than several.
...
Duet's `send_prompt` failure message is the house pattern:
...
Duet's `ask_human` queued-response is the house mini-context:
...
7. Model-facing text carries none of duet's internal vocabulary — run the familiar-term test on every workflow, duty, gate, and checkpoint name before it ships; the process's *shape* in plain words helps a worker, its internal *names* do not.
