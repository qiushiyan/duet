...
Reference for designing duet's agent prompts and tool surfaces, distilled 2026-06-11 from Anthropic's published guidance and first applied in the substrate spike (`src/spike/q11.ts`). Consult this when writing or revising any orchestrator/worker prompt, tool definition, or tool result — the surfaces it governs live in `src/orchestrator/briefs.ts` (the system prompt and phase briefs) and `src/orchestrator/tools.ts` (tool descriptions, results, errors).
