...
Several snippets carry named elicitation frameworks tuned to their task: the rabbit-holes hunt and design-it-twice in `write-spec` (find what could quietly eat the build; arrive at an interface rather than settling for the first one), the pre-mortem in `review-spec` / `review-plan` / `consultant-contract` (prospective hindsight surfaces risks that reading forward misses), red-team-the-tests and definition-of-wrong in `consultant-contract`, answer-first (BLUF) structure in `ceo-summary`, and the review-lens stance (`collaboration/review-lens.md`, vendored) that every code-review lens reads first — `review-implementation` / `review-direct` / `review-and-fix`: the strategic step-back (the reviewer's local-optimum bias: ask whether a reshape dissolves the problem before endorsing the local patch), the additive-bias bar, Chesterton's fence.
...
The methodology snippets cite greenflag's vendored lessons, and **read depth is a decision, not a union**: read a lesson deeply where its decisions get made, skim its `## The bar` where they are only constrained. `write-spec` reads the design lessons closely — it commits the module structure, the interfaces, and the seams — and skims the testing bars, because it names *which* behaviors matter and what gets faked where, not how to write the tests. `start-plan` and the build seeds invert that. `review-implementation` reads the review lens closely — the stance whose calls it makes — and skims the design bar only: the reviewer judges a shape it didn't decide, so that lesson is its lens and the vocabulary its findings are written in; the stance prose itself (the step-back, the additive-bias bar, seam cleanliness, preparatory refactoring) lives in `collaboration/review-lens.md` rather than restated per snippet. A snippet that cites everything has stopped deciding.
