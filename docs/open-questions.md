...
Q13 (triage precision), Q16 (worker schema), Q19 (run-level budget), and Q20 (pre-authorization) all await evidence from more runs — review each run's `notes.md` against them. Q17 (codex-as-orchestrator) waits for someone to actually want the configuration. Q21 (async interactive `send_prompt`) is settled on A′ but stays open behind its revisit trigger. New questions land here when runs produce them.
