...
- **The relay arc, first live run** (`automation-design.md` §"Phases and gates" — the relay arc; §"Consultant checkpoints"). Built as the workflow vocabulary's first proof — one registry entry plus its snippets — and test-verified; not yet live-run. The first serious relay run watches four bets: **fixer-mode adversariality** — the maker-vs-critic dialectic collapses into sequential build-then-judge, so do judge-fixes hold quality without the respond-review exchange, and does the fixer over-rewrite (the "understand what the builder did first" guardrail's real test)? **Criss-cross context loss** — does the judge, reviewing from the committed doc alone, miss nuance the planning loop settled (if so, the fix is a richer handoff or seed, not a posture change)? **The one-round discipline** — does cap 1 plus delta follow-ups absorb a verify self-heal without thrashing the round rail? And the **escalation valve** — does a genuine design disagreement actually escalate rather than get patched over? A consultant binding is the natural companion on this arc (its verify is the only fully independent pass over work the reviewer both graded and edited); watch whether a run without one feels under-checked.
