...
**Why it matters.** On the interactive orchestrator host a blocking `send_prompt` freezes the session for the minutes a worker runs (a CC turn can't yield with a `tool_use` outstanding), defeating the "conversation is the channel" value of running CC as the orchestrator. Three designs answered it: **A′** dispatches the turn as a background promise inside the run-scoped MCP server and collects it later with `check_turns`; **B** runs each turn as a detached child writing its result to disk, surviving a session quit and reconnect; **C** a bounded-blocking single tool. The trade is robustness vs. machinery: A′ is light but a turn in flight dies with the session (orphaned, recovered by `duet takeover`); B survives quit/reconnect but needs prompt-staging across a process boundary, result files, child lifecycle and cleanup, and a second concurrency model.

**Current belief.** A′ — chosen and built (spec/plan: `docs/specs/2026-06-21-async-interactive-send-prompt.md`, `docs/plans/2026-06-21-async-interactive-send-prompt.md`; test-verified, live end-to-end + environment smoke tests pending). For the *attended* arc a turn lost to a deliberate quit is a re-fire, not a silent overnight failure, so B's machinery isn't justified; A′ adds only a small durable pending-turn record and honest orphan recovery, pulling in nothing from B. C was declined — it keeps freezing the session for the cap window and still needs A's background ownership to avoid wasting a non-idempotent turn.

**What would change the answer (the revisit trigger).** Mid-turn session quits become common, or overnight orphans misfire — then B's survive-the-quit robustness earns its cost. Transcript-tail result reconstruction on reconnect (rebuild a `WorkerTurn` from on-disk JSONL) is the lighter intermediate, recorded as a later enhancement of A′'s reconnect path, not a jump to B.
...
**Why it matters.** The consultant (2026-06-22, `docs/automation-design.md` §"The three roles" / §"Consultant checkpoints") is a *bet* on a bet: that a deliberately low-context, ephemeral, cross-family reviewer challenges product framing where the embedded reviewer — invested in the run's accumulated context — is strong on execution and blind to premise. The mechanism is built and test-verified, but its **value is unproven**: no live consultant run has executed, so whether its bet audits actually catch wrong premises (rather than restating the obvious, or firing `high` on non-issues and stalling an overnight run via the severity hold) is untested. Cost is real — an extra worker turn at each of three checkpoints, on a second model family.
