...
- The phase order: onboard → frame → spec → plan → implement → review → docs → PR. (This is the *observed human* flow — the evidence greenflag was drawn from. greenflag's shipped `full` arc since collapsed the docs→PR tail into one `finish` phase, 2026-06-26; the current arc is in `docs/automation-design.md`.)
...
6. **A recurring CEO-reframe move, previously in nobody's library.** "Can you step back and give me a more CEO-facing description of how we plan to design this…" — `b7487993` 07:22:37Z, `a463ad80` 07:12:21Z, `e9607005` 05:12:52Z. Formalized 2026-06-11 as the `ceo-summary` snippet (final-stage variant; `docs/automation-design.md` §"The final-gate packet and the CEO summary").
