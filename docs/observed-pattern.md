...
4. **The implementation review loop ran only once** in this session ("go ahead and proceed" at turn 13). The user's general description says 2–3 rounds. The human stopped early because remaining points were minor. The orchestrator handles this with a fixed per-phase cap (3 for SPEC and IMPL review, 2 for PLAN) that surfaces to the human at the next phase boundary for the ship/another-round decision — see `docs/automation-design.md` §"Loop-exit rule".
