...
duet has no visual way to see a workflow's shape or a run's live position. `duet status` is line-oriented text, and the compiled workflow — stages, phases, gates and their postures, duty bindings, round caps, every materialized default — is legible only by reading JSON or source. Build a visualization surface: one on-demand CLI command (working name `duet graph`; a better name is a design-time call) rendering three views over one pure view model.

1. **Workflow blueprint** — `duet graph --workflow <name>`: the compiled workflow before any run exists. Stages and phases in order; each gate with its default posture (attended / auto-cross); each stage's duty pair with its resolved default binding (provider, model, effort); round caps; continuity edges; consultant checkpoints when a consultant would be bound. Project-composed workflows render through the same resolution `duet workflows` uses. The point of this view: every default made visible at a glance.

2. **Run view** — `duet graph [runId]`: the run's frozen workflow and manifest with live position overlaid. Completed phases marked done; the current phase highlighted in color; future phases dimmed. Gates show what actually happened or will: attended crossing, auto-cross under standing authority, a hold. Loop rounds render as used/cap. A parked question or pending gate is called out with its next command. On-demand snapshot — re-run to refresh.

3. **Execution trace** — `duet graph [runId] --trace`: what actually happened, from the run's logs. Per phase: the turn sequence with duty, snippet tag, and duration; compactions; retries; steers. Divergences from the expected shape are flagged — out-of-order snippet flows, rounds past the cap, unexpected phase re-entries. This is the drift detector: a planning-stage ordering bug (one snippet's results arriving gated behind another's) was once found only by hand-reading transcripts; the trace should surface that class mechanically.
...
- duet workflows are linear pipelines with loops and gates — not general DAGs. Resist generic graph-layout machinery; the diagram is a pipeline with annotations.
...
- Drive it for real: `duet graph --workflow blueprint` and `--workflow relay`; `duet graph <this run's own id>` from inside this worktree mid-run (the run dir is under `.duet/runs/`); `--trace` against this run's own record once phases have completed. Paste a render into the PR description.
