...
**The elegant part — gateless needs no change to the severity-hold mechanism.** The only highs that can reach a gateless gate are: (a) bet-audit highs — *gone*, the checkpoints don't fire; (b) the orchestrator's own product/direction calls — already *carried forward as recommendations* on a pre-authorized run (the existing brief discipline, automation-design §"Gate pre-authorization"; `ask_human` reserved for the would-make-downstream-work-throwaway case); (c) verify highs — now produced *only* when stuck after self-heal, which we *want* to hold even in gateless. So `highDecisionsAt`/`driveToQuiescence`/`enterAfk` stay byte-for-byte; the partition is achieved by *what produces holds* (checkpoint gating + the carry-forward prompt + the self-heal loop), not by classifying decisions. No per-decision `kind` field — the taxonomy we rejected (see Out of scope) is unnecessary. The safe-degradation property holds: if the orchestrator records a product high anyway, the worst case is a flagged stop, never a silent proceed.
...
- **Trust gradient** — each rail placed correctly. The verify *re-run* and the *hold-if-stuck* are **structure** (objective, un-gameable). The implementer's fix-or-leave judgment and the orchestrator's carry-forward of product calls are **text** (steerable) — legitimate because the structural backstop catches a wrong call. We never parse prose for a guarantee.
