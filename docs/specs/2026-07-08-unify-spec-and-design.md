...
**Status:** shipped 2026-07-08. The durable facts are distilled into `CONTEXT.md` (the spec/plan artifact vocabulary), `docs/automation-design.md` (§"The workflow vocabulary" — ordered documents and the derivations that replaced the artifact-name branches; §"Consultant checkpoints" — contract placement derived from whether a committed document precedes the phase), `docs/engineering.md` (the module map), and `docs/corpus-runbook.md` (the retired-vocabulary cohort). This document is kept as the dated record of *why*, and of the alternatives that lost.

**One refinement past the recommendation below.** The *Target shape* section recommends the derived facts (`hasUpstreamDoc`, `isHandoffPhase`) live as **fields on the compiled `PhaseSemantics`**. Code review found that one step short: a stored fact a frozen workflow could contradict, needing a coherence validator to catch the lie, when `handoffGateOf` already derives "planning's last phase" from the frozen phase list beside it. The shipped shape drops both fields and the validator; the facts are **pure functions over the phase list** (`isHandoffPhase(wf, phase)` / `hasUpstreamDoc(wf, phase)` in `registry/workflows.ts`), asked on demand. The recommendation's *reasoning* holds — A's authored vocabulary, no port; only its persistence was tightened.

**A coupling CI cannot see.** `write-spec`'s design-it-twice discipline depends on the vendored `design-it-twice.md` teaching the *solo, constraint-first* path — an earlier revision prescribed spawning parallel sub-agents, which a worker cannot do. The re-vendor shipped in the same commit, but `tests/snippets.test.ts` checks only that a cited lesson path *resolves*, never its content: a future `pnpm vendor-lessons` that reverts that sentence would break `write-spec` with the suite still green. Re-verify the solo-path wording survives any lessons re-sync.
