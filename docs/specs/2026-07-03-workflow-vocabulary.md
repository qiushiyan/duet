...
**Status: shipped (2026-07-03, branch `workflow-vocabulary`) — distilled into `docs/automation-design.md` §"The workflow vocabulary" (+ the relay arc), `docs/engineering.md` (the code shape), and `docs/prompting-and-tool-design.md` (the single-world rule); this file stays as the dated proposal record.** The stage-0 document for duet's repositioning: the shipped arcs become _expressions in a small composition vocabulary_, and a fourth arc (working name **relay**) ships as the vocabulary's first proof. Cross-checked 2026-07-03 by an independent GPT-5.5 design pass over the same problem (blind to this doc, compare-notes after); its valid deltas are folded in below — notably the two-owner tail split, the write-authority resolver, provider-qualified session ids, and the action catalog. Written deliberately ahead of the comprehensive doc pass — the doc-debt ledger at the bottom is the explicit IOU (docs-lead requires the divergence be recorded, never silent). Scope is the big picture: the repositioning, the grammar/vocabulary split, the block inventory, the relay arc, and the one-PR plan. Per-module detail stays in the code and lands in `docs/engineering.md` at the doc pass.
...
**Vocabulary — the composable inventory, closed by rule:** a knob value exists only when duet ships hand-written prompt support for it (brief fragments + snippets) _and_ a shipped arc exercises it. Load-time validation (the `validateRegistry` pattern, extended) rejects a composition that uses an unsupported combination. Speculative knobs — parameterizing anything that doesn't vary across the shipped arcs — are refused on sight; every vocabulary entry is earned by an arc that ships.

## The single-world rule — prompts under knobs

Composability must never leak into what a model reads. **Every rendered prompt describes exactly one world**: the model sees only the instructions that apply to this run's actual configuration, and could not tell from its prompt that other blocks, postures, or arcs exist. Two binding principles:

1. **Never encode knob-conditionals in prose.** No rendered brief or snippet ever says "if the review posture is fixer, …; otherwise …". Conditional prose makes the model parse configuration state, hedges the instruction, and spends its attention on branches that don't apply — it is a patch, not an instruction (the Resources bar: prompts read top-down as clear, natural instructions). The registry's semantics decide; the prose asserts.
2. **Branch in the composer, not the prompt.** All conditioning happens at render time: the statechart knows the phase, the registry row carries the semantics, and the renderer (T5) selects dedicated fragments for the branch. Where knob values diverge in what the worker should *do*, each value gets its own hand-written fragment or snippet — `review-and-fix` is a dedicated snippet, not `review-direct` plus a conditional paragraph — and prose is shared only when genuinely identical across values. Prefer forking a fragment over parameterizing it into hedged generality; the closed vocabulary keeps the fork count bounded, because knob values are few and each is earned by a shipped arc.

The codebase already practices this where it branches today — `resetForImplStep(claudeImplementer)` selects the claude-vs-codex compaction prose in code, and rir's brief carries its own worked examples rather than a shared example with caveats. The rule names that practice and makes it binding for every new knob; the parity fixture rule (one pinned fixture per conditional branch) is its test-side enforcement.
...
The two tail owners are deliberately **separate knobs** (`buildTailOwner`, `finishOwner`) even though relay sets both to the reviewer: "the finishing tail" as one concept would blur the phase boundary — reconcile-docs and ceo-summary are the tail of _implement_ (before Ship, before verify), pr-description and the PR open are _finish_ — and collapsing them risks moving verify ahead of a later mutating turn.
...
**Every block and knob value owns its snippets.** A block is not just machinery — it brings its prompt material: `frame` brings `think-holistic` + `compare-notes`; `doc-loop × spec` brings the `write-spec`/`review-spec`/`update-spec`(-again) family; an `entrySeed` brings its seed snippet; a review posture brings its loop family. A phase's effective snippet list is the **union of what its semantics contribute**, not a hand-authored list — the mechanics and the migration proof are decision T7.
...
2. **Effective binding per stage.** `implementerModelFor` (returns a model string, claude-only by v1 design) generalizes to `effectiveBindingFor(bindings, role, workflow, phase) → RoleBinding`, consumed by `createWorkers` _before_ its provider branch. `parseImplOverride` drops the claude-only guard; the override key generalizes from `impl` to `build` on any worker binding (`impl` accepted as an alias on the implementer — judgment call, no compat burden yet). The replace-don't-mutate discipline on `DEFAULT_BINDINGS` carries over.
...
7. **Brief composers fold into registry-driven rendering.** `orchestrator-prompts.ts` is already half-factored (shared step builders; per-(workflow,phase) composer functions; the `phaseBriefBuilders` dispatch). The composers become one renderer over the registry row + fragment library. **Worked-example blocks stay per-arc data** — examples are the most arc-specific, least composable prose; only the _steps_ become knob-conditional fragments.
...
Two further build-time checks, recorded so they aren't rediscovered mid-PR: (1) **Compaction guidance is implementer-centric prose today** (`compact-for-impl`/`compact-for-review` address the builder), while relay moves the long-lived claude session to the *reviewer*. The fixer's session is born fresh at the handoff and lives one review cycle plus the tail, so it may never reach the bands — confirm the metering and salvage ladder are genuinely role-agnostic, and give the fixer world its own compaction stance in fragments only if a real run shows it's needed (earned, not speculative). (2) **The orchestrator's role descriptions assume a read-only reviewer** in two places: the dynamic system prompt (becomes posture-aware with T5, per the single-world rule) and the static `prompts/orchestrator-identity.md` (serves only the attended pre-handoff arc, where relay's reviewer *is* still read-only — verify its prose stays workflow-safe rather than asserting reviewer read-only-ness as a run-wide fact).
...
**T1 — Session identity: derive the reset, don't event it.** The handoff session reset is _not_ a lifecycle step. `sessionIdFor(state, role)` (`src/roles.ts`) is already the single resume-site read on both hosts (the blocking path and the turn-dispatcher); it grows the phase and compares the stored record's provider against the effective binding's provider for that phase — mismatch ⇒ `undefined` ⇒ the next send mints fresh, exactly the existing ephemeral-consultant mechanism. No hook in `lifecycle.ts`, no crash window (a crash before/after the "reset" changes nothing — the answer re-derives), idempotent, and correct even if a run is resumed mid-arc by an older flow. The `sessionResets` ledger entry lands in `settleTurn` (which already loads-fresh and writes the session): when the settling turn's provider differs from the prior record's, record the reset. Legacy compatibility is a **parse-don't-validate** boundary: `loadRunState` normalizes a legacy bare-string session to `{provider: bindings[role].provider, id}` once at load, so every downstream reader sees one shape and the two-maps-drifting failure mode (a sibling `sessionProviders` map) is never representable. The record is also the single provider source for **every session consumer beyond resume**: `duet takeover`'s choice of resume command (`claude --resume` vs `codex exec resume`), `sessions.ts`'s transcript location (`~/.claude/projects/` vs `~/.codex/sessions/`), and the doctor/status health probes all read the record's provider — never re-derived from the base binding, which the post-handoff override makes wrong for a switched role. Without this, a manual takeover of relay's build implementer would run the wrong CLI.

**T2 — Write authority: one resolver, three consumers.** `writeAuthorityFor(state, phase, role, action)` — the static `POLICY` table remains the default beneath it; the fixer posture (from the phase's semantics + the run's arc) is the only thing that widens it, and only for the reviewer, post-handoff. Its consumers: (1) `settleTurn`'s `verifiedAt`-staleness key (`tools.ts:544` — today `readOnlyFor(role)`; `state` and `phase` are already in `settleTurn`'s signature, so this is a call-site change, not a restructuring); (2) the two `runTurn` dispatch sites that pass the `readOnly` role-intent hint (`tools.ts:1419`, `turn-dispatcher.ts:224`); (3) the fixer-mode rails (may-write). The `action` parameter earns its place here: fixer write authority is **action-scoped, never a phase-blanket** — `review-and-fix` and the tail actions may write, but a `review-midpoint` turn under the fixer posture stays guidance-only (mid-build, the builder is the sole writer; early correction is folded in by the builder, not patched under it — two interleaved writers on one mid-flight build would wreck the builder's mental model of its own tree). The consultant's contract/verify write relaxation stays **prompt-scoped** as today — it never flips this flag — so the author-never-commits premise is untouched.

**T3 — Placement rule for the new data: semantics in the registry, behavior metadata in code, prose in TOML.** The trust gradient applied to the vocabulary. The `semantics` sub-object is registry data (`phases.ts`, load-validated). The action catalog is a **code map keyed by snippet key** (beside the registry, pinned by `tests/snippets.test.ts`: every catalog key exists in the library, every behavior-bearing key is cataloged) — deliberately _not_ fields in `snippets.toml`, because the override layers replace bodies per-key and a user override must never be able to change behavior (`countsReviewRound`, may-write). `snippets.toml` stays prose-only. `countsReviewRound(role, tag)`'s call sites don't move; its implementation reads the catalog instead of the tag prefix.

**T4 — Effective binding: absorb, don't parallel.** `effectiveBindingFor(bindings, role, workflow, phase) → RoleBinding` **replaces** `implementerModelFor` (its one non-view caller is `createWorkers`; the stats view adapts) rather than sitting beside it — two resolvers answering "who runs this turn" is the drift the deletion test exists to catch. `createWorkers` consumes it _before_ the provider branch, so the codex-vs-claude construction falls out per phase. All rejection guards stay at the config boundary (`loadRunConfig`): override-on-interactive-transport rejected, orchestrator claude-only unchanged; past the boundary the binding is trustworthy and no downstream site re-checks.
...
**T7 — Blocks own their snippets: derive the lists, don't list them.** Today each phase row hand-lists its snippets, and repeated phases copy lists across arcs (design's `implement` is full's plus one substitution). Under the vocabulary the list becomes a **derivation**: each semantic element carries its snippet family — the `frame` block brings `think-holistic` + `compare-notes`; `artifact: spec|plan|design` brings its write/review/update(-again) family; an `entrySeed` brings its one seed snippet; `reviewPosture: critique` brings the handoff + `review-implementation`/`respond-review`(-again) family, `writable` brings `handoff-direct`/`review-direct`/`apply-review`, `fixer` brings `review-and-fix`; `midpoint: judgment` brings the midpoint trio; the tails bring `reconcile-docs` / `pr-description` — and the phase's effective list is the union in a canonical order. The attachment maps are **explicit data, never name-convention parsing** (the library predates the convention: plan's draft snippet is `start-plan`, not `write-plan`). This extends a derivation the consultant already uses (`consultantSnippetFor` from the checkpoint mode) to the whole library, and it makes the closed-vocabulary rule structural: `validateRegistry` now checks that every knob value a shipped arc uses has its snippet family present — "a knob value ships its prompt support" stops being prose. The per-row `snippets:` arrays are **deleted** (the deletion test), and the parity harness's snippet pins prove the derived lists byte-identical to today's hand lists for the three arcs. Where the derivation *cannot* reproduce a hand list, that is a discovery, not a failure — the vocabulary is missing a knob. One known case going in: `ceo-summary` rides full/design's `implement` but not rir's; the derivation forces that implicit difference to become explicit (a ship-packet knob, or a documented build-block default) — decide it in the build when the mismatch is concrete, not speculatively here. User configuration is untouched and the split stays sharp: the override layers swap snippet *bodies* per-key, never membership — membership is behavior and lives registry-tier (T3's placement rule). A stage-4 external arc file adding project snippets to a phase (additive membership) stays trigger-gated with the rest of stage 4.

## The snippet library's shape

`snippets.toml` is ~1,050 lines and grows with every arc; relay adds several more snippets. The runtime never suffers — loading is package-relative and `list_snippets` is phase-focused by design, so no model is ever served the whole library — which means the cost is **authoring-time**: an agent editing one snippet chunks through a thousand unrelated lines, an accidental full read burns tokens, and every arc's prose changes land in one file.

Decision: **split the shipped library into a `snippets/` directory of block-named TOML files** mirroring the vocabulary — `frame.toml`, one per doc-loop artifact (`doc-spec.toml`, `doc-plan.toml`, `doc-design.toml`), `build.toml`, `finish.toml`, `anytime.toml`, `consultant.toml`; exact boundaries are an implementation call behind the loader. `src/snippets.ts` reads the directory and merges into the one served library; key uniqueness is validated at load (fail loud on a duplicate); the served output is byte-identical, so the parity harness's snippet pins prove the migration. File layout then navigates by the same mental model as the language — an agent touching the fixer posture opens `build.toml` and nothing else.

What deliberately does not change: keys stay globally unique and remain the only identity (no filename-derived keys); the **override layers stay single-file per-key** (`~/.config/duet/snippets.toml`, `<repo>/.duet/snippets.toml` — sparse by nature); the brief **fragments stay in TypeScript** (they interpolate run state and are behavior-coupled — two prose corpora, two homes: the worker-facing library in TOML, the orchestrator-facing fragments in code); the tabtype hand-sync is per-key and unaffected. Rejected alternatives: the status quo (the authoring cost compounds with every arc), and one-file-per-snippet (maximal edit locality, but cross-snippet coherence edits — retuning every `review-*` lens at once — become N-file sweeps, and filename-vs-key dual identity invites drift).
...
1. **This document.**
2. **Parity harness first**: snapshot-pin rendered briefs, effective snippet lists, and machine shapes for full/design/rir — _before any refactor commit_. The run-state matrix covers consultant on/off, gateless, spec-entry, attended vs pre-authorized gate copy, prior gate auto-crossed vs explicitly approved, acceptance contract absent/draft/frozen/verified, and claude vs codex implementer (the compaction prose differs). Not the full Cartesian product — the rule is **every conditional branch in `orchestrator-prompts.ts` has at least one pinned fixture** before the renderer refactor. The bar is **byte-identical**, stricter than green tests. Wording improvements discovered mid-refactor are deferred, never folded in.
3. **Split the snippet library by block** (§"The snippet library's shape") — behavior-preserving; the parity harness's served-library pins stay green, and relay's new snippets are then born in the right files.
4. **Vocabulary fields in the registry** (no consumer changes — a no-op commit, load-validated), then the T7 snippet-family derivation replacing the per-row `snippets:` arrays — the parity harness's snippet pins prove the derived lists byte-identical, and any mismatch is a missing knob surfacing, handled here rather than papered over.
5. **Fold the brief composers** into registry-driven rendering; parity stays green. Fragment authoring follows the single-world rule.
6. **Run-aware role policy** + the `verifiedAt` keying fix.
7. **Stage-scoped bindings**: `effectiveBindingFor`, provider switch, session reset at handoff.
8. **The relay arc as pure data** + its snippets (`review-and-fix`, the fixer-tail variants, the fresh-seed entry) + tests. This commit is the proof: a new workflow lands as one `WORKFLOWS` entry plus snippets, no new composer function.
9. **README status line** (relay: built, test-verified, not live-run) + a one-line CLAUDE.md pointer to this doc. The comprehensive pass waits.
...
For state machine changes, consult .agents/skills/xstate-v5/SKILL.md.

For designing and changing new snippets: consult /prompt-engineering and docs/prompting-and-tool-design.md.

For changing skills, read /writing-great-skills.

All prompt changes and skill changes should read top-down as clear, natural instructions instead of just random patches. Prefer structural rewrites rather than patching.
