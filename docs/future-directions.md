...
**Eval/replay harness — the trigger has fired.** Re-run a recorded phase's orchestrator against scripted workers (voice logs replayed through the `WorkerProvider` seam, injectable by design) and diff the routing choices — snippets picked, adaptations, triage calls, convergence judgment — against the original. The original trigger was "the first prompt change that makes a run worse and we can't tell why"; the 2026-07-06 snippet batch (rabbit-holes, pre-mortems, BLUF) shipped research-backed but unmeasured, which is that trigger in slow motion. What it compounds: prompt changes become A/B evidence instead of faith, the open calibration questions (triage precision, context bands, the consultant's value) gain an instrument other than live runs, robustness regressions become replayable, and a future user's transcript becomes a reproducible case. The first slice worth having is small: replay one recorded phase, diff snippet tags and terminal calls; scoring comes later.
