...
For **one run in its project**, two product surfaces read its record. `duet stats <runId>` is the effort surface — per-phase elapsed/worker time and the by-tag breakdown. `duet grade <runId>` is the **calibration ritual**: a 60-second end-of-run walkthrough of the run's reconstructed decision points — gate stops (attended / auto-crossed / held-high) and queued questions — each presented with enough inline context to take a plain **right/wrong** verdict (was the stop worth the interruption; should the auto-cross have stopped you). The human also asserts **missed stops** — a stop that should have happened but left no trace. Verdicts persist as the additive `grades` ledger and mirror like every other; `--list` previews the points read-only. Both surfaces are read-time reconstructions over the same state-ledger-plus-log discipline — the grading core reads state first and the log only where state is silent — and neither ever lets a missing log silently shrink coverage: a degradation is a surfaced note, not a rosy rate.
...
node scripts/corpus/grade-precision.ts        # triage precision: over-flag / under-flag rates by workflow / gate / kind / month
```
