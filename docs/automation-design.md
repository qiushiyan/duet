...
The tool surface is **host-neutral**: the seven tools live in one registry independent of any SDK, hosted by two thin adapters — the in-process Agent SDK server (the default) and a standard stdio MCP server (`docs/engineering.md`, the "Host-neutral kernel" pattern). The same handlers and rails serve both, so the orchestrator can be driven from a separate process at behavioral parity — the enabling architecture for the in-CC-orchestrator direction (`docs/future-directions.md` §A), built and parity-tested but not the user-visible orchestrator itself.
