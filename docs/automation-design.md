...
| `send_prompt(role, tag, body)` | Send a prompt to the implementer or reviewer. `tag` names the source snippet (`"custom"` when composed from scratch); `body` is the final text. Every call logs the tag and the body, so adaptation drift is auditable. Independent turns to different roles run concurrently when issued as parallel calls (the frame phase's two analyses); a second turn to the same role while one is in flight is refused — one session is one conversation. **Host-divergent completion:** headless, it blocks and returns the worker's response; on the interactive host it dispatches the turn into the background and returns at once, the result collected later with `check_turns`. |
| `check_turns()` | **Interactive host only.** Collect the results of turns dispatched by `send_prompt` — the worker's text (or a failed turn's prescribed recovery), with the same bookkeeping a blocking turn would have done, plus any pending steers. Instant: it delivers whatever has settled, names any role still running, and reopens a collected role; a phase cannot advance while a dispatched turn is uncollected. (`duet status --wait` wakes the moment a turn settles.) |
