...
**Verified now (no live auth, over `FakePane` + tmpdir transcripts + fake timers):** the parser (plain/tool-using/incomplete/compact/cut-line/nonce-isolation), the config knob and its codex/role rejections and override merge, the cost flag, the full driving loop, both timeout paths (readiness and post-injection watch) with teardown asserted, nonce correlation among decoys, the resumed-append and slug-fallback cases, and the loud no-candidate / ambiguous failures.
