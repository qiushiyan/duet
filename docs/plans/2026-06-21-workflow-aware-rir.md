...
> **Historical (pre-`finish`).** Dated record from before the 2026-06-26 collapse of the `docs`/`pr`/`open` tail into one `finish` phase (open-then-review, PR-auto-open-by-default, `overnight` as full's default posture). Its Full-arc walks still name the `docs`/`pr`/`open` states — the topology of their time; the current arc lives in [`../automation-design.md`](../automation-design.md).
