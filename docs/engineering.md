...
| `src/harness/lifecycle.ts` | The detached `_drive` process, pid guard + `killDriver`, quiescence loop, `gates_at` auto-cross, and `probeRunPosition` (where a run is) | The only place machine actors run for real |
...
| `src/run-store.ts` | `.duet/runs/<id>/` persistence: state hint, machine snapshot, voice logs, notes; the CLI↔driver input-staging handshake; the steer store (`steers/`, file-per-steer, rename-consume); the context hint + its `context/<voice>` sidecars; the abandoned marker + `purgeRun` teardown | Atomic writes; `state.json` is a HINT — transcripts are truth |
